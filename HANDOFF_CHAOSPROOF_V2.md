...
FaultRay（旧FaultRay）の商用化に向けた大規模機能追加を実装してください。
...
│   └── store.py         (139行) — シナリオ永続ストア（~/.faultray/）
├── plugins/             # プラグインシステム
...
**ファイル**: `src/faultray/simulator/security_engine.py`（新規）
...
**ファイル**: `src/faultray/simulator/advisor_engine.py`（新規）
...
**ファイル**: `src/faultray/api/insurance_api.py`（新規）
...
### 既存APIサーバーへの統合
`src/faultray/api/server.py` に新ルーターを追加:
```python
...
**ファイル**: `src/faultray/reporter/compliance.py`（既存拡張）+ 新規ファイル
...
**ファイル**: `src/faultray/reporter/executive_report.py`（新規）
...
```bash
# P0
faultray security <yaml>                           # セキュリティ耐性分析
faultray cost <yaml>                               # コスト影響分析
faultray monte-carlo <yaml> -n 10000               # モンテカルロ可用性分析
...
faultray score-v2 <yaml>                           # Resilience Score v2

# P2
faultray predict <yaml>                            # 障害予測
faultray gameday <yaml> --plan gameday.yaml        # ゲームデイ事前検証
faultray markov <yaml>                             # マルコフ連鎖分析
faultray advise <yaml>                             # カオステスト推奨
```
...
`src/faultray/model/components.py` に追加するフィールド:
...
**YAML loader** (`src/faultray/model/loader.py`) も対応する新フィールドのパースを追加。
