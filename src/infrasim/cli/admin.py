...
def demo(
    ...
    if web:
        ...
        set_graph(graph)
        console.print(f"\n[green]Starting FaultZero dashboard at http://{host}:{port}[/]")
...
def serve(
    ...
    Examples:
        ...
        faultzero serve --model my-model.json

        # Custom host and port
        faultzero serve --host 127.0.0.1 --port 3000

        # Enable Prometheus continuous monitoring
        faultzero serve --prometheus-url http://prometheus:9090 --prometheus-interval 30
    """
    import os
    ...
    if prometheus_url:
        os.environ["FAULTZERO_PROMETHEUS_URL"] = prometheus_url
        os.environ["FAULTZERO_PROMETHEUS_INTERVAL"] = str(prometheus_interval)
        console.print(
            f"[cyan]Prometheus monitoring enabled: {prometheus_url} "
            f"(interval={prometheus_interval}s)[/]"
        )

    console.print(f"[green]Starting FaultZero dashboard at http://{host}:{port}[/]")
    uvicorn.run("infrasim.api.server:app", host=host, port=port, log_level="info")
...
def report(
    ...
    Examples:
        ...
        faultzero report --model my-model.json --output results.html
    """
...
def plan(
    ...
    Examples:
        ...
        faultzero plan my-model.json

        # Set target score and budget
        faultzero plan --target-score 95 --budget 50000

        # JSON output for automation
        faultzero plan --json

        # Export plan as HTML
        faultzero plan --html plan-report.html
    """
    ...
    console.print(Panel(
        summary_text,
        title="[bold]FaultZero Remediation Plan[/]",
        border_style="cyan",
    ))
...
<head>
<meta charset="utf-8">
<title>FaultZero Remediation Plan</title>
<style>
...
<body>
<h1>FaultZero Remediation Plan</h1>
<div class="summary">
...
"""
    ...
    html_content += """
<hr>
<p><em>Generated by FaultZero Remediation Planner</em></p>
</body>
</html>"""
...
def quickstart(
    ...
    console.print("\n[bold cyan]Welcome to FaultZero![/] Let's build your infrastructure model.\n")
    ...
    console.print(Panel(
        report_text,
        title="[bold]FaultZero Quick Report[/]",
        border_style="cyan",
    ))
