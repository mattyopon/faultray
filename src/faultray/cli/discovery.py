...
@app.command()
def tf_import(
    tf_state: Path | None = typer.Option(
        None, "--state", "-s", help="Path to terraform.tfstate file"
    ),
    tf_dir: Path | None = typer.Option(
...
def tf_plan(
    ...
    if len(after_graph.components) > 0:
        ...
        console.print(
            "\n[yellow]No HTML report produced: the planned ('after') infrastructure "
            "has no components to simulate.[/]"
        )
...
def _iac_gen_remediate(
    ...
    for f in plan.files:
        if Path(f.path).is_absolute():
            console.print(f"[red]Refusing to write outside output dir: {f.path}[/]")
            raise typer.Exit(1)
        file_path = (base / f.path).resolve()
        if not (file_path == base or base in file_path.parents):
            console.print(f"[red]Refusing to write outside output dir: {f.path}[/]")
            raise typer.Exit(1)
...
def _iac_gen_export(
    ...
    def _safe_id(name: str) -> str:
        """Convert a component name/id to a safe, unique Terraform resource identifier.

        Guarantees a non-empty result and avoids collisions between distinct
        inputs that normalise to the same string by appending a short stable
        hash of the original value.
        """
        normalized = re.sub(r"[^a-zA-Z0-9_]", "_", name).lower().strip("_")
        suffix = hashlib.sha256(name.encode("utf-8")).hexdigest()[:8]
        base = normalized or "component"
        # Apply length limit AFTER reserving room for the uniqueness suffix.
        return f"{base[:48]}_{suffix}"
...
def _comment_safe(value: object) -> str:
    """Sanitise a value for safe inclusion in a single-line ``#`` HCL comment.

    Strips newlines and control characters so user-controlled values (e.g. a
    component name) cannot break out of the comment and inject HCL.
    """
    text = str(value)
...
def _generic_export_block(tf_id: str, comp: "Component", comp_type: str) -> str:
    ...
    return (
        f'# {_comment_safe(comp.name)} ({comp_type})\n'
        f'# host={_comment_safe(comp.host or "unknown")}, port={comp.port}, replicas={comp.replicas}\n'
